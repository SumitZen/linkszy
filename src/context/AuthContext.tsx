...
const STORAGE_KEY = 'linkszy_users';
const SESSION_KEY = 'linkszy_session';
...
async function syncProfileToAppwrite(updated: User): Promise<void> {
    ...
    try {
        ...
        const error = err as { code?: number; message?: string };
        console.error('❌ Appwrite Sync Failed:', error.message);
        if (error.code === 404) delete docIdCache[userId];
        throw err;
    }
}
...
export function AuthProvider({ children }: { children: ReactNode }) {
    const [user, setUser] = useState<User | null>(() => {
        ...
        if (session) {
            try {
                ...
                if (parsed.bio === 'Hey there! I use Linkszy.') parsed.bio = '';
                return parsed;
            } catch { localStorage.removeItem(SESSION_KEY); }
        }
        return null;
    });
    const [isLoading, setIsLoading] = useState(APPWRITE_READY);
            ...
            account.get().then(async (awUser) => {
                try {
                    ...
                    if (res.documents.length > 0) {
                        ...
                        const saved = localStorage.getItem(`linkszy_profile_${awUser.$id}`);
                    ...
                    localStorage.setItem(`linkszy_profile_${awUser.$id}`, JSON.stringify(freshUser));
                } catch {
    ...
    const login = async (email: string, password: string) => {
        if (APPWRITE_READY) {
            try {
                ...
                if (finalUser.bio === 'Hey there! I use Linkszy.') finalUser.bio = '';
                setUser(finalUser);
    ...
    const signup = async (name: string, email: string, password: string, plan: User['plan'] = 'free') => {
        if (APPWRITE_READY) {
            try {
                ...
                localStorage.setItem(`linkszy_profile_${u.id}`, JSON.stringify(u));
    ...
    const updateUser = (updates: Partial<User>) => {
        ...
        localStorage.setItem(`linkszy_profile_${prev.id}`, JSON.stringify(updated));
        ...
        if (APPWRITE_READY && prev.id) {
            ...
            syncDebounceTimer = setTimeout(() => {
                syncProfileToAppwrite(updated).catch(e => {
                    ...
                    setUser(prev);
                    localStorage.setItem(`linkszy_profile_${prev.id}`, JSON.stringify(prev));
