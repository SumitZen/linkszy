...
import type { LinkItem, Block } from '../context/AuthContext';
...
export default function PublicProfile() {
        ...
        return (
            <div style={{ 
                ...
                <div style={{ textAlign: 'center' }}>
                    ...
                    <div style={{ fontSize: '0.9rem', opacity: 0.6, fontWeight: 500, letterSpacing: '0.05em' }}>LINKSZY</div>
                </div>
    ...
    const forceParseJSON = (data: unknown): unknown[] => {
        if (!data) return [];
        try {
    ...
    const links: LinkItem[] = forceParseJSON(profile?.links) as LinkItem[];
    const blocks: Block[] = forceParseJSON(profile?.blocks) as Block[];

    const isPreview = new URLSearchParams(window.location.search).get('preview') === 'true';
    const userPlan = profile.plan || 'free';
    const totalContent = [...links.map(l => ({ ...l, itemType: 'link' as const })), ...blocks.map(b => ({ ...b, itemType: 'block' as const }))];
        ...
        <div style={{
            ...
            <div style={{
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 16, width: '100%' }}>
                    {(visibleContent as ProfileItem[]).filter(item => item.enabled !== false).map((item) => {
